...
public interface VisitorRepository extends JpaRepository<Visitor, Long> {
	...
	public Optional<Visitor> findByMachineIdAndVisitorId(@Param(MACHINE_ID) Long machineId,
			@Param(VISITOR_ID) Long visitorId);

	@Query("Select v from Visitor v where v.amusementPark.id = :amusementParkId and v.id = :visitorId")
	public Optional<Visitor> findByAmusementParkIdAndVisitorId(@Param(AMUSEMENT_PARK_ID) Long amusementParkId,
			@Param(VISITOR_ID) Long visitorId);
