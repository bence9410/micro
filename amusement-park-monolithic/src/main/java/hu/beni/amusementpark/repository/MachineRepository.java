...
public interface MachineRepository extends JpaRepository<Machine, Long> {
	...
	Optional<Long> sumAreaByAmusementParkId(@Param(AMUSEMENT_PARK_ID) Long amusementParkId);

	@Query("Select m from Machine m where m.amusementPark.id = :amusementParkId and m.id = :machineId")
	Optional<Machine> findByAmusementParkIdAndMachineId(@Param(AMUSEMENT_PARK_ID) Long amusementParkId,
			@Param(MACHINE_ID) Long machineId);

	@Query("Select m from Machine m where m.amusementPark.id = :amusementParkId")
	List<Machine> findAllByAmusementParkId(@Param(AMUSEMENT_PARK_ID) Long amusementParkId);
