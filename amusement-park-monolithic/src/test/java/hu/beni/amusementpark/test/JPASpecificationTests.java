...
public class JPASpecificationTests {
	...
	public void findOneNotRetunsUniqueResult() {
		amusementParkRepository.findOne(like("name", BASE_AMUSEMENT_PARK.getName() + "1%"));
	}

	@Test
	public void findOneCapitalGreaterThan() {
		int capital = 28000;
		AmusementPark result = amusementParkRepository.findOne(greaterThan("capital", capital)).get();
	...
	public void findOneNameLikeAndCapitalGreaterThan() {
		...
		Specification<AmusementPark> spec = like("name", BASE_AMUSEMENT_PARK.getName() + "1%");
		spec = spec.and(greaterThan("capital", capital));

		AmusementPark amusementPark = amusementParkRepository.findOne(spec).get();
	...
	public void findOneNameLikeAndCapitalGreaterThanAndAddressCityLike() {
		...
		spec = spec.and(like("name", BASE_AMUSEMENT_PARK.getName() + "1%")).and(greaterThan("capital", capital))
				.and(like("address.city", "Buda%"));
	...
	public void findAllWithMachineForChildren() {
		...
		Specification<AmusementPark> spec = (Root<AmusementPark> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
			...
			subQuery.where(cb.equal(root.get("id"), subRoot.get("amusementPark").get("id")),
					cb.lt(subRoot.get("minimumRequiredAge"), 18));
		...
		spec = (Root<AmusementPark> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
			Join<AmusementPark, Machine> joinMachines = root.join("machines");
			joinMachines.on(cb.lt(joinMachines.get("minimumRequiredAge"), 18));
			query.distinct(true);
			query.orderBy(cb.asc(root.get("id")));
			return null;
		};
	...
	private <T> Specification<T> like(String fieldName, String param) {
		return (Root<T> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> cb.like(createPath(root, fieldName),
				param);
	}

	private <T> Specification<T> greaterThan(String fieldName, int param) {
