...
public class OracleRowNumberPartitionTests {
	...
	private static final String SELECT_ALL_VISITOR_USERNAME_AND_LAST_GUEST_BOOK_REGISTRY = "select visitor.username, "
	...
	public void test() {
		...
		assertEquals(g1.getVisitor().getUsername(), row[0]);
		assertEquals(g1.getTextOfRegistry(), row[1]);

		row = result.get(1);

		assertEquals(g2.getVisitor().getUsername(), row[0]);
	...
	private List<Object[]> executeQuery() {
		return entityManager.createNativeQuery(SELECT_ALL_VISITOR_USERNAME_AND_LAST_GUEST_BOOK_REGISTRY)
				.getResultList();
	}
