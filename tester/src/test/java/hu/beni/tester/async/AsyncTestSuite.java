...
import static hu.beni.tester.constant.Constants.PAGE_SIZE;
...
public class AsyncTestSuite {
	...
	public CompletableFuture<DeleteTime> deleteAllPark(HttpHeaders headers) {
		...
		do {
			...
			page.getContent().stream().map(this::parkToContentId)
	...
	public CompletableFuture<SumAndTime> sumAmusementParksCapital(HttpHeaders headers) {
		...
		do {
			...
			sum += page.getContent().stream()
					.mapToInt(amusementParkResource -> amusementParkResource.getContent().getCapital()).sum();
		} while (!page.isLast());
	...
	public CompletableFuture<VisitorStuffTime> visitSomeStuffInEveryPark(HttpHeaders headers) {
		...
		do {
			long tenParkStart = now();
			page = client.getAmusementParks(pageIndex++, PAGE_SIZE, headers);
			page.getContent().stream().map(this::parkToContentId).forEach(
					amusementParkId -> visitEverythingInAPark(amusementParkId, visitorId, headers, oneParkTimes));
			tenParkTimes.add(millisFrom(tenParkStart));
		} while (!page.isLast());
		return CompletableFuture.completedFuture(new VisitorStuffTime(millisFrom(start), tenParkTimes, oneParkTimes));
	}

	private void visitEverythingInAPark(Long amusementParkId, Long visitorId, HttpHeaders headers,
			List<Long> oneParkTimes) {
		long startPark = now();
		client.enterPark(amusementParkId, visitorId, headers);
		client.getMachineIdsByAmusementParkId(amusementParkId, headers).stream().map(this::machineToContentId)
				.forEach(machineId -> {
	...
	public CompletableFuture<SumAndTime> sumVisitorsSpendingMoney(HttpHeaders headers) {
		...
		do {
			...
			sum += page.getContent().stream()
					.mapToInt(visitorResource -> visitorResource.getContent().getSpendingMoney()).sum();
		} while (!page.isLast());
		return CompletableFuture.completedFuture(new SumAndTime(sum, millisFrom(start)));
	}
	...
	public CompletableFuture<DeleteTime> deleteAllVisitor(HttpHeaders headers) {
		...
		do {
			...
			page.getContent().stream().map(this::visitorToContentId)
